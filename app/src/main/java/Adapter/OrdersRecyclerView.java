...
public class OrdersRecyclerView extends RecyclerView.Adapter<OrdersRecyclerView.ViewHolder> {
    private Context context;
    private ArrayList<CartItem> cartItems;
    private FirebaseDatabase firebaseDatabase = FirebaseDatabase.getInstance();
    private FirebaseAuth firebaseAuth = FirebaseAuth.getInstance();

    public OrdersRecyclerView(Context context, ArrayList<CartItem> cartItems) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        firebaseDatabase.getReference().child("products").child(cartItem.getProductId()).get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                ...
                if (product.getEndingTime().after(new Date())){
                    cartItems.remove(cartItem);
                    notifyItemRemoved(position);
                }
                holder.dateAdded.setText(formatter.format(product.getEndingTime()));
                ...
                holder.dateDue.setText("Status: On Process");
